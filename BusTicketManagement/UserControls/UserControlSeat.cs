    ...
    public partial class UserControlSeat : UserControl
    {
        ...
        private void buttonA1_Click_1(object sender, EventArgs e)
        {
            ...
            if (statusA1 == "free" && buttonYellowA1 == 0)
            {
                if (seatList.Count > 3)
                {
                    MessageBox.Show("You have reached maxiumu limit");
                }
                else
                {
                    seatList.Add("a1");
        ...
        private void button25_Click(object sender, EventArgs e)
        {
            foreach (string el in seatList)
            {
                try
                {
                    ...
                    string query = @"UPDATE seat_status SET status = 'booked' WHERE Seat_no='" + el + "' and trip_id=" + trip_id_number + "";
                    SqlCommand cmd = new SqlCommand(query, con);

                    cmd.ExecuteNonQuery();

                    //panelSignInForm.Controls.Clear();
                    //panelSignInForm.Controls.Add(this);
            ...
            MessageBox.Show("booked for 1 min");
        ...
        private void OnTimedEvent(object source , EventArgs e) {
            ...
            try
            {
                foreach (string el in seatList)
                {
                    ...
                    string query = @"UPDATE seat_status SET status = 'free' WHERE Seat_no='" + el + "' and trip_id=" + trip_id_number + "";
                    SqlCommand cmd = new SqlCommand(query, con);
